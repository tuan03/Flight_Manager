...
class Menu{
    ...
    void render_menu(HanhKhach* mb, int vt){
        // render khung menu
...
class Edit{
    ...
    void render_menu(HanhKhach* mb, Flight_Manager* qlcb){
...
class View_Customer{
    private:
        ...
        Customer::State state = Customer::State::HOME; // mặc định ở Child_Route Home
        Customer::Menu menu_customer; //route menu
        Customer::Edit edit_customer; //route edit



        int vi_tri_hover_on_table = -1; //vị trí hover trên table

        HanhKhach* temp = nullptr; // lấy con trỏ máy bay ở vị trí đang hover rồi click

        int so_luong_data = 0; // số lượng máy bay

        int route_customer_width[5]{100,400,250,500,250};
    ...
    void handleEvent(SDL_Event e,Name_Box& current_route, ListBox& menu,bool& is_home, bool& quit) {
        ...
        if(this->state == Customer::State::HOME){
            is_home = true;
        switch (e.type) {
                case SDL_MOUSEMOTION:
                this->vi_tri_hover_on_table = this->get_vitri(this->mouse_X,this->mouse_Y,qlcb->getListHK().so_hanh_khach()); // 0 - 9
                break;
                case SDL_MOUSEBUTTONDOWN: // sự kiện nhấn vào các box
                ...
                // end handle
                    ...
                    if(this->vi_tri_hover_on_table != -1){ // nếu nhấn vào, mà đnag hover vào line trong table thì bật menu.
                            this->state = Customer::State::MENU;
                    }
                    break;
                ...
                is_home = false;
            if(state == Customer::State::MENU){
                menu_customer.handleEvent(e,this->state,mouse_X,mouse_Y,quit);
            } else
            if(state == Customer::State::EDIT){
                edit_customer.handleEvent(e,this->state,mouse_X,mouse_Y,quit);
            }
            }
    }
    
    void render() {
        this->so_luong_data = qlcb->getListHK().so_hanh_khach(); // lấy số lượng máy bay

        myscreen->render_table(5,route_customer_width,route_customer_name_cot); 
        ...
        if(state == Customer::State::MENU){
                menu_customer.render_menu(temp,vi_tri_hover_on_table);
        } else 
        if(state == Customer::State::EDIT){
                edit_customer.render_menu(temp,this->qlcb);
        }
   ...
   void render_next_prev(){
    ...
    void render_line_data(int stt, int start,HanhKhach* mb){
        SDL_Rect rect;
        rect = {X_START_TABLE, Y_START_TABLE + (stt-start)*50 ,route_customer_width[0],50};
                ...
                rect.x += rect.w; 
                rect.w = route_customer_width[1];
                this->myscreen->render_Text(mb->getHo(), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_customer_width[2];
                this->myscreen->render_Text(mb->getTen(), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_customer_width[3];
                this->myscreen->render_Text(mb->getSoCMND(), rect,{0,0,0,255},true);
        ...
        void duyet_tree(HanhKhach* p,int& stt, int start, int& so_line_render) {
            if (p == nullptr) {
                return;
            }
            

            duyet_tree(p->getLeft(),stt,start,so_line_render);
            int temp_stt = stt++;
            if(temp_stt >= start && temp_stt <= start+9){
                
                this->render_line_data(temp_stt,start,p);
                so_line_render++;
            }
            duyet_tree(p->getRight(),stt,start,so_line_render);
        }
    void render_data(){
        ...
        HanhKhach* current = this->qlcb->getListHK().getRoot();
        this->duyet_tree(current,stt,start, so_line_render);
