...
class View_Flight{
    private:
        ...
        Flight::State state = Flight::State::HOME; // mặc định ở Child_Route Home
    ...
    void handleEvent(SDL_Event e,Name_Box& current_route, ListBox& menu,bool& is_home, bool& quit) {
        ...
        if(this->state == Flight::State::HOME){
            is_home = true;
        switch (e.type) {
                case SDL_MOUSEMOTION:
                this->vi_tri_hover_on_table = this->get_vitri(this->mouse_X,this->mouse_Y,qlcb->getListCB().get_so_luong_cb()); // 0 - 9
                break;
                case SDL_MOUSEBUTTONDOWN: // sự kiện nhấn vào các box
            ...
            input_ma_cb.handleInput_IN_HOA_SO_KHONG_CACH(e,mouse_X,mouse_Y);

            } else {
                is_home = false;
            if(state == Flight::State::MENU){
                menu_flight.handleEvent(e,this->state,mouse_X,mouse_Y,quit);
            } else
            if(state == Flight::State::EDIT){
                edit_flight.handleEvent(e,this->state,mouse_X,mouse_Y,quit);
            }
            }
    }
    
    void render() {
        this->so_luong_data = qlcb->getListCB().get_so_luong_cb(); // lấy số lượng máy bay
        ...
        this->myscreen->render_Text("Mã CB:",rect_temp,{0,0,0},true);
        input_ma_cb.render(); // render ô input nhập số hiệu máy bay
   ...
   void render_next_prev(){
    ...
    void render_line_data(int stt, int start,ChuyenBay* mb){
        SDL_Rect rect;
        rect = {X_START_TABLE, Y_START_TABLE + (stt-start)*50 ,route_flight_width[0],50};
                ...
                this->myscreen->render_Text(mb->get_ma_so_cb(), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_flight_width[2];
                this->myscreen->render_Text(mb->get_san_bay_den(), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_flight_width[3];
                this->myscreen->render_Text(mb->get_thoi_gian_bay().to_string(), rect,{0,0,0,255},true);
                        
                rect.x += rect.w; 
                rect.w = route_flight_width[4];
                this->myscreen->render_Text(std::to_string(mb->get_trang_thai_cb()), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_flight_width[5];
                this->myscreen->render_Text(mb->get_so_hieu_mb(), rect,{0,0,0,255},true);

                rect.x += rect.w; 
                rect.w = route_flight_width[6];
                this->myscreen->render_Text(std::to_string(mb->dem_so_ve_con_trong()), rect,{0,0,0,255},true);
    ...
    void render_data(){
        ...
        ChuyenBay* mb = nullptr;
        ChuyenBay* current = this->qlcb->getListCB().get_head();
        while (current != nullptr)
        {
            mb = current;
            if(Func_Global::check_prefix(mb->get_ma_so_cb(),this->ma_cb.c_str())){
